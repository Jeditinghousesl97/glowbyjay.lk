...
<style>
        ...
        *,*::before,*::after{border-radius:0 !important}
        ...
        .gallery-thumb.active{border-color:var(--primary);box-shadow:0 0 0 2px rgba(182,138,45,.16)}
        ...
        .btn-order-now{background:var(--btn-ordernow-bg, var(--primary));color:var(--btn-ordernow-text, #fff);box-shadow:0 12px 24px rgba(182,138,45,.18)}
    ...
    <style>
        ...
        #orderModal .field input:focus,#orderModal .field textarea:focus,#orderModal .field select:focus{outline:none;border-color:rgba(182,138,45,.42);box-shadow:0 0 0 3px rgba(182,138,45,.10)}
...
<div id="paymentMethodSheet" class="payment-sheet-overlay" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.45);z-index:3100;align-items:center;justify-content:center;padding:16px;" onclick="closePaymentMethodSheet()">
    <div class="payment-sheet" onclick="event.stopPropagation()" style="width:min(100%,720px);max-height:calc(100vh - 32px);max-height:calc(100dvh - 32px);overflow-y:auto;-webkit-overflow-scrolling:touch;overscroll-behavior:contain;background:var(--surface);border-radius:18px;padding:18px;box-shadow:0 20px 40px rgba(0,0,0,.18);">
        ...
        <div style="display:flex;justify-content:space-between;gap:16px;align-items:flex-start;margin-bottom:16px;">
            <div>
                <div style="font-size:11px;letter-spacing:.2em;text-transform:uppercase;color:#a0a0a0;margin-bottom:6px;">Choose Payment Method</div>
                <h3 style="margin:0;font-size:22px;font-family:sans-serif;">Select how you want to order</h3>
                <div class="payment-method-toggle">
                    <button type="button" id="payNowToggleBtn" class="payment-method-toggle-btn pay-now" onclick="setPaymentMethodCategory('payNow')">Pay Now</button>
                    <button type="button" id="payLaterToggleBtn" class="payment-method-toggle-btn pay-later" onclick="setPaymentMethodCategory('payLater')">Pay Later</button>
                </div>
            </div>
...
<div id="cartConfirmOverlay" class="cart-confirm-overlay" onclick="closeCartConfirm()">
    <div class="cart-confirm" onclick="event.stopPropagation()">
        <div class="cart-confirm-head">
            ...
            <p id="cartConfirmText">Product Successfully added to the cart</p>
        </div>
...
<script>
    ...
    function showCartConfirm(message) {
        ...
        if (!overlay) return;
        if (text) text.textContent = message || 'Product Successfully added to the cart';
    ...
    function setPaymentMethodCategory(category) {
        const sheet = document.getElementById('paymentMethodSheet');
        if (!sheet) return;
        const payNowBtn = document.getElementById('payNowToggleBtn');
        const payLaterBtn = document.getElementById('payLaterToggleBtn');
