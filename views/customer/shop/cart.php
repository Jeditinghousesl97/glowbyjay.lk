...
(function () {
    ...
    function updateQty(index, qty, line) { return fetch(baseUrl + 'cart/updateQty', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ index:index, qty:qty }) }).then(r=>r.json()).then(function (data) { if (!data || !data.success) throw new Error((data && data.message) || 'Unable to update cart'); line.dataset.qty = String(qty); if (items[index]) items[index].qty = qty; syncLine(line); updateGlobalCount(data.count || 0); recalc(); }); }
    function removeLine(index, line) { return fetch(baseUrl + 'cart/remove', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ index:index }) }).then(r=>r.json()).then(function (data) { if (!data || !data.success) throw new Error((data && data.message) || 'Unable to remove item'); if (index >= 0 && index < items.length) items.splice(index, 1); line.remove(); updateGlobalCount(data.count || 0); recalc(); toast('Cart item removed.', 'success'); if (!document.querySelector('[data-cart-line]')) window.location.reload(); }); }
    ...
    document.addEventListener('click', function (event) {
        ...
        if (btn.matches('[data-cart-clear]')) { event.preventDefault(); fetch(baseUrl + 'cart/clear', { method:'POST', headers:{'Content-Type':'application/json'} }).then(r=>r.json()).then(function (data) { if (!data || !data.success) throw new Error('Unable to clear cart'); updateGlobalCount(0); window.location.reload(); }).catch(e=>toast(e.message, 'error')); }
