<?php
$kokoEndpoint = $kokoEndpoint ?? '';
$kokoPayload = is_array($kokoPayload ?? null) ? $kokoPayload : [];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        :root{--primary:#b68a2d;--ink:#1c1b1b;--muted:#6d6665}
        *{box-sizing:border-box}
        ...
        .spinner{width:54px;height:54px;border:4px solid rgba(182,138,45,.14);border-top-color:var(--primary);border-radius:50%;margin:0 auto 18px;animation:spin .8s linear infinite}
...
<body>
    <div class="card">
        ...
        <h1>Redirecting to KOKO</h1>
        <p>Please wait while we connect you to the KOKO secure payment page.</p>

        <form id="kokoRedirectForm" action="<?= htmlspecialchars($kokoEndpoint) ?>" method="post" style="display:none;">
        ...
        <div class="fallback">
            <button type="button" onclick="document.getElementById('kokoRedirectForm').submit();">Continue</button>
        </div>
    </div>
